...
router.get('/internal-tasks', [
  ...
  try {
    ...
    if (assignedTo) {
      // Filter tasks where assignedProductIds include assignedTo string
      whereClause.assignedProductIds = { has: assignedTo };
    }
    ...
    // Fetch assigned products user details separately to avoid Prisma limitation
    // Build map from User Id to User fullName
    const allAssignedUserIds = tasks.flatMap(task => task.assignedProductIds || []);
    ...
    // Map tasks to report data with assigned product names
    const reportData = tasks.map(task => {
      ...
      // Map assignedProductIds to user full names
      const assignedProductNames = (task.assignedProductIds || [])
        .map(id => userMap[id])
        .filter(name => !!name);
      ...
      return {
        ...
        assignedProducts: assignedProductNames.join(', '),
        ...
        isExpiringSoon: task.expiryDate &&
          new Date(task.expiryDate) <= new Date(Date.now() + 15 * 24 * 60 * 60 * 1000)
      };
    });
...
// Exchange Tasks Report (Corrected)
router.get('/exchange-tasks', [
  ...
  try {
    ...
    const { dateFrom, dateTo, exchangeName, approvalStatus } = req.query;
    ...
    // Collect all assigned product user IDs for batch fetching
    const allAssignedUserIds = tasks.flatMap(task => task.assignedProductIds || []);
    const uniqueUserIds = [...new Set(allAssignedUserIds)];
    const assignedUsers = await prisma.user.findMany({
      where: { id: { in: uniqueUserIds } },
...
// Compliance Users Report (Corrected)
router.get('/compliance-users', [
  ...
  try {
    ...
    for (const user of complianceUsers) {
      ...
      if (avgApprovalTimeTasks.length) {
        const totalDays = avgApprovalTimeTasks.reduce((sum, task) => {
          const days = Math.ceil((new Date(task.approvalDate) - new Date(task.createdAt)) / (1000 * 60 * 60 * 24));
          return sum + days;
        }, 0);
        avgApprovalTimeDays = Math.round(totalDays / avgApprovalTimeTasks.length);
      }
    ...
    const summary = {
      ...
      totalTasksAssigned: reportData.reduce((sum, u) => sum + u.totalAssigned, 0),
      totalPending: reportData.reduce((sum, u) => sum + u.pending, 0),
      totalApproved: reportData.reduce((sum, u) => sum + u.approved, 0),
      avgProductivity: Math.round(reportData.reduce((sum, u) => sum + u.productivityScore, 0) / reportData.length)
    };
...
// Product Users Report (Corrected)
router.get('/product-users', [
  ...
  try {
    ...
    for (const user of productUsers) {
      const tasksCreatedCount = await prisma.task.count({
        where: {
          createdBy: user.id,
      ...
      const versionsUploadedCount = await prisma.version.count({
        where: {
      ...
      const commentsAddedCount = await prisma.comment.count({
        where: {
          authorId: user.id,
          ...(dateFrom && { createdAt: { gte: new Date(dateFrom) } }),
          ...(dateTo && { createdAt: { lte: new Date(dateTo) } })
      ...
      const publishedTasksCount = await prisma.task.count({
        where: {
          OR: [{ createdBy: user.id }, { assignedProductIds: { has: user.id } }],
          status: 'PUBLISHED',
      ...
      const reopenedTasksCount = await prisma.auditLog.count({
        where: {
          performedBy: user.id,
      ...
      const activityScore = tasksCreatedCount + versionsUploadedCount + commentsAddedCount;
      ...
      reportData.push({
        ...
        versionsUploaded: versionsUploadedCount,
        commentsAdded: commentsAddedCount,
    ...
    const summary = {
      totalUsers: reportData.length,
      totalTasksCreated: reportData.reduce((sum, u) => sum + u.tasksCreated, 0),
      totalVersionsUploaded: reportData.reduce((sum, u) => sum + u.versionsUploaded, 0),
      totalCommentsAdded: reportData.reduce((sum, u) => sum + u.commentsAdded, 0),
      totalPublished: reportData.reduce((sum, u) => sum + u.publishedTasks, 0)
    };
  ...
  try {
    ...
    const expiringTasks = await prisma.task.findMany({
      ...
      include: {
        ...
        exchangeApprovals: {
          select: { exchangeName: true, expiryDate: true, referenceNumber: true }
        }
      }
    });

    // Fetch all assignedProductIds for user names
    const allAssignedUserIds = expiringTasks.flatMap(task => task.assignedProductIds || []);
    const uniqueUserIds = [...new Set(allAssignedUserIds)];
    const assignedUsers = await prisma.user.findMany({
      where: { id: { in: uniqueUserIds } },
    ...
    const reportData = expiringTasks.map(task => {
      ...
      return {
        ...
        createdBy: task.creator.fullName,
        assignedProducts: assignedProductInfo,
    ...
    res.json({ summary, data: reportData, cutoffDays: days });
  } catch (error) {
...
router.get('/daily-movement', [
  ...
  try {
    ...
    const movements = await prisma.auditLog.findMany({
      ...
      include: {
        performedByUser: { select: { fullName: true, username: true, role: true } },
        task: { select: { uin: true, title: true, status: true, taskType: true } }
      },
    ...
    movements.forEach(movement => {
      summary.movementsByAction[movement.action] = (summary.movementsByAction[movement.action] || 0) + 1;

      const userKey = movement.performedByUser.fullName;
      summary.movementsByUser[userKey] = (summary.movementsByUser[userKey] || 0) + 1;

      const hour = new Date(movement.timestamp).getHours();
      summary.movementsByHour[hour] = (summary.movementsByHour[hour] || 0) + 1;
    });
    ...
    res.json({
      ...
      movements: movements.map(m => ({
        ...
        action: m.action,
        details: m.details,
        performedBy: m.performedByUser.fullName,
        userRole: m.performedByUser.role,
        task: m.task ? {
          uin: m.task.uin,
          title: m.task.title,
...
router.get('/rejected-tasks', [
  ...
  try {
    ...
    let whereClause = { status: { in: ['CLOSED_INTERNAL', 'CLOSED_EXCHANGE'] } };
    ...
    // Fetch all assignedProductIds for user fullNames
    const allAssignedUserIds = rejectedTasks.flatMap(task => task.assignedProductIds || []);
    const uniqueUserIds = [...new Set(allAssignedUserIds)];
    const assignedUsers = await prisma.user.findMany({
      where: { id: { in: uniqueUserIds } },
      select: { id: true, fullName: true }
    ...
    const reportData = rejectedTasks.map(task => ({
      ...
      title: task.title,
      createdBy: task.creator.fullName,
      assignedProducts: (task.assignedProductIds || []).map(id => userMap[id]).filter(n => n).join(', '),
      assignedCompliance: task.assignedCompliance?.fullName || 'Not assigned',
      status: task.status,
      createdAt: task.createdAt,
      closureDate: task.closureDate,
      closureComments: task.closureComments,
      daysActive: task.closureDate ? Math.ceil((new Date(task.closureDate) - new Date(task.createdAt)) / (1000 * 60 * 60 * 24)) : null
    }));
