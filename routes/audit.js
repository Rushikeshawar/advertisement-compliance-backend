...
router.get('/', [
  ...
  try {
    ...
    if (performedBy) whereClause.performedBy = performedBy; // FIXED: was performedById
    ...
    const formattedLogs = auditLogs.map(log => ({
      ...
      details: log.details,
      performedBy: {
        name: log.performedByUser?.fullName || 'Unknown',
        username: log.performedByUser?.username || 'Unknown',
        role: log.performedByUser?.role || 'Unknown'
      },
      task: log.task ? {
        uin: log.task.uin,
...
router.get('/user/:userId', [
  ...
  try {
    ...
    let whereClause = { performedBy: userId }; // FIXED: was performedById
...
router.get('/stats', [
  ...
  try {
    ...
    const userStats = await prisma.auditLog.groupBy({
      by: ['performedBy'], // FIXED: was performedById
      where: whereClause,
      _count: {
        performedBy: true // FIXED: was performedById
      },
      orderBy: {
        _count: {
          performedBy: 'desc' // FIXED: was performedById
        }
      },
    ...
    const userIds = userStats.map(stat => stat.performedBy); // FIXED: was performedById
    ...
    const userActivityMap = userStats.map(stat => {
      const user = users.find(u => u.id === stat.performedBy); // FIXED: was performedById
      return {
        user: user ? {
          name: user.fullName,
          username: user.username,
        ...
        activityCount: stat._count.performedBy // FIXED: was performedById
      };
    });
    ...
    const [totalLogs, totalUsers, totalTasks] = await Promise.all([
      ...
      prisma.auditLog.findMany({
        where: whereClause,
        distinct: ['performedBy'] // FIXED: was performedById
...
router.get('/export', [
  ...
  try {
    ...
    const { dateFrom, dateTo, performedBy, action } = req.query;
    let whereClause = {};
    ...
    const exportData = auditLogs.map(log => ({
      ...
      action: log.action,
      details: log.details,
      performedBy: log.performedByUser?.fullName || 'Unknown',
      username: log.performedByUser?.username || 'Unknown',
      role: log.performedByUser?.role || 'Unknown',
